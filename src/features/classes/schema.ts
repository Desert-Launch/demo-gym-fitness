...
// Numbers stay numbers end to end: the inputs hand back valueAsNumber, so the
// schema needs no coercion and the form's input and output types match.
export const classFormSchema = z.object({
  typeId: z.enum(CLASS_TYPE_IDS),
  trainerId: z.string().min(1, "Every class needs a coach."),
  dayOfWeek: z.number({ error: "Pick a day." }).int().min(0).max(6),
  startTime: z
    .string()
  ...
  capacity: z
    .number({ error: "Enter how many spots there are." })
    .int()
